...
/**
 * The Obsidian the desktop suites run against.
 *
 * Set here as well as in `editContext` rather than relying on the spread of
 * `context.desktop`, so the capture project does not depend on which of the two
 * hooks the shared config happens to apply first.
 */
const DESKTOP_OBSIDIAN_VERSION = process.env['OBSIDIAN_VERSION'] ?? 'public-latest';
    ...
    return [
      {
        test: {
          ...
          environmentOptions: {
            obsidianTransport: {
              obsidianVersion: DESKTOP_OBSIDIAN_VERSION,
              type: 'obsidian-cdp'
            }
          },
