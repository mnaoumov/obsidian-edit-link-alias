...
Links in a note's **properties** are covered too, written back through the frontmatter so the YAML stays valid - see [[05 Frontmatter links]].

**How to try it:** `Alt` + click a link (`Option` + click on macOS) to edit it right where it is - see [[04 Alt click a link to edit it]]. The same editors are also on the Command Palette, the editor right-click menu, and the link long-press / Reading-view context menu: **Edit link alias** for an alias-only prompt, **Edit link (URL and alias)** for the two-field pop-up.
